#include "client.h"

int login_request(void) {
    char *responce = mx_strnew(2000);

    if (mx_strlen(log_reg.username) < 1 || mx_strlen(log_reg.password) < 1)
        return 0;
    for (int i = 0; log_reg.username[i]; i++)
        if (log_reg.username[i] < 32 || log_reg.username[i] > 126)
            return 0;
    for (int i = 0; log_reg.password[i]; i++)
        if (log_reg.password[i] < 32 || log_reg.password[i] > 126)
            return 0;

	cJSON *j_login = cJSON_CreateObject();
    cJSON *j_responce = cJSON_CreateObject();
    cJSON_AddItemToObject(j_login, "action", cJSON_CreateString("login"));
    cJSON_AddItemToObject(j_login, "username", cJSON_CreateString(log_reg.username));
    cJSON_AddItemToObject(j_login, "password", cJSON_CreateString(log_reg.password));
    char *jdata = cJSON_Print(j_login);
    write(log_reg.sock_fd, jdata, mx_strlen(jdata));
	cJSON_Delete(j_login);
    free(jdata);

    recv(log_reg.sock_fd, responce, 2000, 0);
    j_responce = cJSON_Parse(responce);
    free(responce);
    cJSON *json_type = cJSON_GetObjectItemCaseSensitive(j_responce, "status");
    if(cJSON_IsTrue(json_type))
        return 1;
    return 0;
}
