...
cJSON *download_message_history(cJSON *j_request, cJSON *j_responce) {
    ...
    if (num > 0) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            msg_count = mx_count_sms(chats_id[i], json_type1->valuestring);
            ...
            msg_id = get_msgs_ids(chats_id[i], msg_count, json_type1->valuestring);
            ...
            msg_time = mx_get_time(msg_id, msg_count);
            for (int j = 0; j < msg_count; j++)
            ...
            is_owner = mx_is_msg_owner(json_type1->valuestring, msg_count, msg_id);
            for (int j = 0; j < msg_count; j++)
            ...
            msg_text = mx_get_msg_text(msg_id, msg_count);
            for (int j = 0; j < msg_count; j++)
