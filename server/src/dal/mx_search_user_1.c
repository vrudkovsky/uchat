    ...
    do
        {
            ...
            while(SQLITE_ROW == sqlite3_step(stmt)) {
                if ((strcmp((const char*)sqlite3_column_text(stmt,1), username_1) == 0 ) &&  strcmp((const char*)sqlite3_column_text(stmt,2), username_2)) {
 					count++;
                }
                if ((strcmp((const char*)sqlite3_column_text(stmt,1), username_2) == 0 ) &&  strcmp((const char*)sqlite3_column_text(stmt,2), username_1)) {
 					count++;
                }
...
static char *search_user_1(const char *username_1, const char *username_2) { //затычка либы
    int rc = 0;
    int count = 0;
    char zSql[]="SELECT * FROM users";
    //printf("*** \tShow DB\t ***\n");
    do
        {
            ...
            while(SQLITE_ROW == sqlite3_step(stmt)) {
                if (strcmp((const char*)sqlite3_column_text(stmt,0), username_1) == 0 ) {
                    rc = sqlite3_finalize(stmt);
...
char *mx_search_user_1(char *id_chat) {
    ...
    char *result = search_user_1(username_11, username_22);
